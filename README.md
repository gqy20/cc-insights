...
每次推送到 main 或提交 PR 都会触发 CI 构建 5 平台二进制（linux/darwin × amd64/arm64 + windows/amd64）。产物是 GitHub Actions artifact，可用 gh CLI 直接下载：

```bash
gh run list --limit 3                               # 查看最近的 CI run
gh run download <run-id>                            # 下载该次 run 的全部平台产物
gh run download <run-id> -n cc-insights-linux-amd64 # 只下载某个平台
```

artifact 临时保留 90 天；正式发布版见 [Releases](https://github.com/gqy20/cc-insights/releases)。
